...
# 上の設定の方が優先度が高いので注意!
echo "# private networks connection setting" >> /etc/postgresql/11/main/pg_hba.conf
echo -e "host\tall\t\tall\t\t192.168.33.10/8\t\tmd5" >> /etc/postgresql/11/main/pg_hba.conf

# この設定を使うなら
# 本番はprivate network以外にdbserverを置かないこと。
sed -i.org "s/^#listen_addresses.*$/listen_addresses=\'*\'/" /etc/postgresql/11/main/postgresql.conf
